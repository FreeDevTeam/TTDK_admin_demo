  ...
  return new Promise((resolve) => {
    ...
    for (let key in data) {
      if (data.hasOwnProperty(key)) {
          newObj[process.env.REACT_APP_KEY_PAYLOAD + key] = data[key]; // Add 'key' prefix to each key
      }
    }
    
    let encryption = newObj
      ...
      .then((result) => {
        ...
        for (let key in decryption) {
          if (decryption.hasOwnProperty(key)) {
              ...
              let newKey = key.slice(process.env.REACT_APP_KEY_PAYLOAD.length);
