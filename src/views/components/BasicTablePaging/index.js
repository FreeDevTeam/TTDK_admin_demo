...
const BasicTablePaging = ({handlePaginations}) => {
    const Enter = 13
  ...
  return (
    <Pagination className="pagination react-paginate separated-pagination pagination-sm justify-content-end pr-3 mt-1">
      <div className="prev-item position-relative">
          <span onClick={() =>moveToPreviousPage()} className='bg_left'>< ChevronLeft /></span>
      </div>
      <div>
        <Input 
      ...
      <div className="next-item position-relative">
          <span onClick={() =>moveToNextPage()} className='bg_right'>< ChevronRight /></span>
      </div>
    </Pagination>
  )
}
