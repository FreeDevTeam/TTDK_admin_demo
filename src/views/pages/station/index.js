...
import { STATION_STATUS_FILTER_ZERO, stationTypes } from '../../../constants/dateFormats';
...
const StationPage = ({ intl, filterParam, apiFilter, setNumber }) => {
  const DefaultFilter = {
    filter: {
    },
    skip: 0,
...
if(dataUser && dataUser.stationType?.length > 1){
  ...
  const serverSideColumns = [
    {
      ...
      cell: (row) => {
        return <a
          className='text-primary text-table'
    ...
    },
    {
      ...
      cell: (row) => {
        ...
        return (
          <div>{totalInspectionLine * MAX_SCHEDULE_PER_INSPECTION_LINE}</div>
        )
      }
    },
    {
  ...
  const onExportExcel = async () => {
    ...
    async function fetchData(NumberPage) {
      ...
      const response = await StationFunctions.getListStation(paramsFilter)
