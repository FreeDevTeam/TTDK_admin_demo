...
const StationPage = ({ intl }) => {
  ...
  const onExportExcel = async () => {
    ...
    async function fetchData(param) {
      ...
      if(paramsFilter.filter.stationStatus !== undefined){
        newData.filter.stationStatus = paramsFilter.filter.stationStatus
      }
      if(paramsFilter.filter.stationArea !== undefined){
        newData.filter.stationArea = paramsFilter.filter.stationArea
      }
      if(paramsFilter.filter.stationType !== undefined){
        newData.filter.stationType = paramsFilter.filter.stationType
      }
      const response = await SationService.getListStation(newData)
