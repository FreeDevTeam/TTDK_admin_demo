...
import addKeyLocalStorage, { storeAllStationsDataToLocal } from "../../../helper/localStorage";
...
import { STATION_TYPE } from './../../../constants/app';
import './index.scss';
...
const stationTypes = [
  ...
  { value: 4, label: 'Đơn vị cứu hộ' },
  { value: 6, label: 'Đơn vị tư vấn' },
]

let newStationType = []
const dataUser = JSON.parse(localStorage.getItem("TTDK_ADMIN_WEB_userData"));
if(dataUser.stationType === 'ALL'){
  DefaultFilter.filter.stationType = ''
}
if(dataUser.stationType?.length === 1){
...
if(dataUser.stationType?.length > 1){
  ...
  newArr.map(item =>{
    let Arr = stationTypes.filter(el => el.value === Number(item))
    Arr.map(op =>{
...
const StationPage = ({ intl }) => {
  ...
  const serverSideColumns = [
    ...
    },
    {
      ...
      cell: (row) => {
        ...
        return (
          <div>{totalInspectionLine * 40}</div>
        )
      }
    },
    {
    ...
    },
    {
      ...
      cell: (row) => {
        ...
        const active = totalInspectionLine * 40
        ...
        return (
          <div>
            {stationType === STATION_TYPE.EXTERNAL
              ? <Badge color='light-success' className='size_text'>{intl.formatMessage({ id: 'external' })}</Badge>
              : stationType === STATION_TYPE.GARAGE
              ? <Badge color='light-danger' className='size_text'>{intl.formatMessage({ id: 'garage' })}</Badge>
              : stationType === STATION_TYPE.INSURANCE
              ? <Badge color='light-info' className='size_text'>{intl.formatMessage({ id: 'insurance_unit' })}</Badge>
              : stationType === STATION_TYPE.HELPSERVICE
              ? <Badge color='light-primary' className='size_text'>{intl.formatMessage({ id: 'help_service' })}</Badge>
              : <Badge color='light-warning' className='size_text'>{intl.formatMessage({ id: 'consulting_unit' })}</Badge>}
          </div>
        )
      }
    },
    ...
    },
    {
      ...
      cell: (row) => {
        ...
        return (
          <MySwitch
            checked={res.some(item => item === true) === true ? true : false}
          />
        )
      }
    },
  ...
  function getData(params, isNoLoading) {
    ...
    if (token) {
      ...
      SationService.getListStation(params, newToken).then((res) => {
          if (res) {
  ...
  function handleUpdateData(item, messageSuccess) {
    SationService.handleUpdateData(item).then(res => {
      if (res) {
  ...
  const onUpdateStationEnableUse = (path, data) => {
    SationService.handleUpdateData(data).then(res => {
      if (res) {
  ...
  const ratioHandler = (value) => {
    ...
    const active = value?.totalInspectionLine * 40
    const total = (totalSchedule / active * 100).toFixed(0)
    return total
  }
  ...
  const onlineHandler = (value) => {
    ...
    return (
      dayCount >= 2 ? intl.formatMessage({ id: 'not' }) : intl.formatMessage({ id: 'yes-' })
    )
  }
  ...
  const onExportExcel = async () => {
    ...
    async function fetchData(NumberPage) {
      paramsFilter.skip = NumberPage * 20
      const response = await SationService.getListStation(paramsFilter)
    ...
    <Fragment>
      <Card>
        <Row className='mx-0 mt-1 mb-50'>
          <Col sm='6' md = '4' lg='3' xs='12' className='d-flex mt-sm-0 mt-1'>
          ...
          {/* <Col sm='2' xs='6' className='mb-1'>
            <Input onChange={(e) => {
              const { name, value } = e.target
              handleFilterChange(name, value)
            }} type='select' value={paramsFilter.filter ? (paramsFilter.filter.availableStatus || '') : ''} name='availableStatus' bsSize='sm' >
              {
                OverLoadOptions.map(item => {
                  return <option value={item.value}>{intl.formatMessage({ id: item.label })}</option>
                })
              }
            </Input>
          ...
          <Col sm='6' md = '4' lg='2' xs='12' className='mb-1'>
            {/* <Input onChange={(e) => {
              ...
              <BasicAutoCompleteDropdown  
                ...
                name='stationType'
                options={newStationType?.length > 0 ? newStationType : stationTypes}
                onChange={({ value }) => handleFilterChange("stationType", value)}
              />
          </Col> : null
          }
          <Col sm='2' md = '2' lg='1' xs='12' className='mb-1'>
