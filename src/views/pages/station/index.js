...
const StationPage = ({ intl }) => {
  ...
  const serverSideColumns = [
    ...
    },
    {
      ...
      maxWidth: '240px',
      cell: (row) => {
        return <a target='_blank' href={`${row.stationUrl.startsWith("http://") ? row.stationUrl : "http://" + row.stationUrl}`}>{row.stationCode}</a>
      }
    },
    {
      name: intl.formatMessage({ id:'hidden' }),
      minWidth: '70px',
      maxWidth: '70px',
      center: true,
      cell: (row) => {
        return (
          <MySwitch
            checked={row.isHidden === 1 ? true : false}
            onChange={e => {
    ...
    {
      name: intl.formatMessage({ id: "stationContractStatus" }),
      maxWidth: '230px',
      minWidth: '230px',
      center: true,
      cell: (row) => {
        const { stationContractStatus } = row
        return intl.formatMessage({ id: CONTRACT_STATUS.find(status => status.value === stationContractStatus)?.label || "" })
      }
    },
    {
      ...
      <Card>
        <Row className='mx-0 mt-1 mb-50'>
          ...
          <Col className='d-flex align-items-center justify-content-sm-end mt-sm-0 mt-1' sm='4'>
            <Label for='search-input' style={{ minWidth: 70, margin: 0 }}>
          ...
          <Col className='d-flex align-items-center justify-content-sm-end' sm="2">
