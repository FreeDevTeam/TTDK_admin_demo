...
import {
  ...
  Button, FormGroup, Form, Modal, ModalHeader, ModalBody, ModalFooter
} from 'reactstrap'
...
const FormSMS = ({ intl }) => {
  ...
  function handleUpdateData(data) {
    if(stationEnableUseSMS === 1 && useUserSMSBrandConfig === 1) {
      if(Object.keys(errors).length === 0) {
        ...
        Service.send({
          method: 'POST', path: 'Stations/updateConfigSMS', data: params, query: null
        }).then(res => {
          if (res) {
    ...
    if(smsType === "VIETTEL") {
      ...
      Service.send({
        ...
        if (result && result.statusCode === 200) {
          ...
          toast.error(intl.formatMessage({ id: "sendSMSTestFailed" }, { type: "" }))
        }
      })
    } else {
      const params = {
  ...
  return (
    <Fragment>
              ...
              <FormGroup row>
                ...
                <FormGroup row>
                  ...
                  <Col sm='5'>
                    ...
                    >
                      <option value="">{intl.formatMessage({id: "selectService"})}</option>
                ...
                <div className='row m-0 mt-3'>
                  ...
                  <FormGroup className='col-12 col-sm-3 p-0'>
                    <Button.Ripple className='w-100' color="primary" disabled={(stationEnableUseSMS === 1) ? false : true} onClick={() => setIsOpenModalTestSMS(true)}>
                      Test SMS
                    </Button.Ripple>
                  </FormGroup>
                </div>
      ...
      <Modal isOpen={isOpenModalTestSMS} toggle={() => setIsOpenModalTestSMS(!isOpenModalTestSMS)}>
        <Form onSubmit={(e) => {
          e.preventDefault()
        ...
        <ModalHeader toggle={() => setIsOpenModalTestSMS(!isOpenModalTestSMS)}>{intl.formatMessage({id:"checkSMS"})}</ModalHeader>
          <ModalBody>
            <label htmlFor='testSMS'>{intl.formatMessage({id:"enterPhoneSendSMS"})}</label>
            <FormGroup>
              <Input
                name="testSMS"
                id="testSMS"
