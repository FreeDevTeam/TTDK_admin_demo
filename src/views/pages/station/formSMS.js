...
const FormSMS = ({ intl }) => {
  ...
  function handleUpdateData(data) {
    if (stationEnableUseSMS === 1) {
      if (Object.keys(errors).length === 0) {
        ...
        SationService.handleUpdateConfigSMS(params).then((res) => {
          if (res) {
  ...
  const onChangeUseCustomSMSBrand = (newValue) => {
    SationService.onChangeUseCustomSMSBrand({
      stationsId: state.stationsId,
  ...
  const onUpdateStationEnableUseSMS = (newValue) => {
    SationService.handleUpdateData({
      id: state.stationsId,
      data: {
  ...
  function handleSendTestSMS(value) {
    ...
    SationService.handleSendTestSMS(params).then((result) => {
