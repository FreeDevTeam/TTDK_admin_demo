...
import { MoreVertical, Edit, Lock, Delete, DollarSign, Database, Shield, RefreshCcw, ChevronLeft } from 'react-feather'
...
const FormStation = ({ intl }) => {
  ...
  const onChangeAdStatus = (newStatus) => {
    Service.send({
      method: 'POST', path: 'Stations/enableAdsForStation', data: newStatus, query: null
    }).then(res => {
      if (res) {
        const { statusCode, } = res
        if (statusCode === 200) {
          setUserData({...userData, stationsEnableAd: newStatus.stationsEnableAd})
        } else {
          toast.warn(intl.formatMessage({ id:'actionFailed' }, {action: intl.formatMessage({id: "update"})}))
        }
      }
    ...
    Service.send({
      ...
      if (res) {
        ...
        if (statusCode === 200) {
          ...
          toast.warn(intl.formatMessage({ id:'actionFailed' }, {action: intl.formatMessage({id: "update"}) + "Ads"}))
        }
      }
    })
  }
    ...
    <Fragment>
      <Card  >
        ...
        <div className="accountAdmin">
          <h1 className="accountAdmin__title">  {userData.stationsId ? 
            intl.formatMessage({id: "edit"}) :
            intl.formatMessage({id: "add"})
          } {" "} {intl.formatMessage({id: "info"}, {type: intl.formatMessage({id: "stations"})})}</h1>
          <Row>
            <Col sm="12" md="7">
            <Form onSubmit={handleSubmit((data) => {
              const newData ={
                  ...data,
                  stationsLogo: previewArr && previewArr[0]? previewArr[0].imageUrl : undefined
                }
                if(userData.stationsId){
                  ...
                  if(previewLeftBanner && previewLeftBanner[0] && previewLeftBanner[0].imageUrl) {
                    handleUpdateBanner("Stations/updateLeftAdBanner",{
                      "stationsId": userData.stationsId,
                      "stationsCustomAdBannerLeft": previewLeftBanner[0].imageUrl
                    })
                  }
                  if(previewRightBanner && previewRightBanner[0] && previewRightBanner[0].imageUrl) {
                    handleUpdateBanner("Stations/updateRightAdBanner",{
                      "stationsId": userData.stationsId,
                      "stationsCustomAdBannerRight": previewRightBanner[0].imageUrl
                    })
                  }
                }else{
                 ...
                 <Label sm="3" for='stationsName'>
                  ...
                  <FormGroup row>
                  <Label sm="3" for='stationsLogo'>Logo</Label>
                  ...
                  <FormGroup row>
                  <Label sm="3" for='stationsHotline'>Hotline</Label>
                  ...
                  <div className="row">
                    <Label sm="3">
                      {intl.formatMessage({ id: "stationUseCustomSMSBrand" }, {type: intl.formatMessage({id: "advertising"}).toLowerCase() })}
                    </Label>

                    <Col sm='9'>
                      <MySwitch
                        checked={userData && userData.stationsEnableAd === 1 ? true : false}
                        onChange={(e) => {
                          onChangeAdStatus({
                            stationsId: userData.stationsId,
                  ...
                  
                  {
                    ...
                    <>
                      <FormGroup row>
                        ...
                        <Col sm='9'>
                        <FileUploaderBasic setPreviewArr={setPreviewLeftBanner} previewArr={previewLeftBanner} />
                        </Col>
                      </FormGroup>
                      ...
                      <FormGroup row>
                        ...
                        <Col sm='9'>
                        <FileUploaderBasic setPreviewArr={setPreviewRightBanner} previewArr={previewRightBanner} />
                        </Col>
                      </FormGroup>
                    </>
                    ) : ( <> </>)
