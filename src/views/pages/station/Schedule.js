...
const scheduleOptions = [
  { value: '', label: 'all_schedule' },
  { value: 0, label: 'new-appointment' },
  { value: 10, label: 'appointment-confirmed' },
  { value: 20, label: 'appointment-cancelled' },
  { value: 30, label: 'appointment-closed' }
]
  ...
  const serverSideColumns = [
    ...
    },
    {
      ...
      cell: (row) => {
        ...
        return (
          <div>
            {vehicleType.toString() === VEHICLE_TYPE.CAR
              ? intl.formatMessage({ id: "car" })
              : vehicleType.toString() === VEHICLE_TYPE.OTHER 
      ...
      <Col className="mb-1"
        ...
        <Col sm='2' xs='6'>
          <Input onChange={(e) => {
            const { name, value } = e.target
            handleFilterChange(name, value)
          }} type='select' value={paramsFilter.filter ? (paramsFilter.filter.CustomerScheduleStatus || '') : ''} name='CustomerScheduleStatus' 
          bsSize='md' >
            {
