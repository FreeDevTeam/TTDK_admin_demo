...
const Order = ({ intl }) => {
  ...
  const columns = [
        ...
        return (
          <div>
            ...
            <div className="order-text">
              Trạng thái : <Badge color={paramsOrder?.color}>{intl.formatMessage({ id: paramsOrder?.label })}</Badge>
            </div>
      ...
      cell: (row) => {
        return (
          <div>
            ...
            <div className="order-text">
              Trạng thái : <Badge color="success">{row?.trangThaiTT}</Badge>
            </div>
    ...
    },
    {
      ...
      maxWidth: '200px',
      cell: (row) => {
        return <div>{row?.processNote}</div>
      }
    },
    {
      ...
      cell: (row) => (
        <span>
          ...
          <span className="pr-1 w-25" onClick={(e) => {
            e.preventDefault()
            history.push(`/pages/detail-order/${row.orderId}`)
          }}>
            {<Edit size={16} />}
  ...
  return (
    <Fragment>
      ...
      <Modal isOpen={openOne} toggle={() => setOpenOne(false)} size="md" className={`modal-dialog-centered `}>
        <ModalHeader toggle={() => setOpenOne(false)}>{intl.formatMessage({ id: 'note' })}</ModalHeader>
        <ModalBody>
          <Form
