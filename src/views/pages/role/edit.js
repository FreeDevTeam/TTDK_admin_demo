  ...
  const serverSideColumns = [
    {
      ...
      cell : (row) =>{
        ...
        return (
          <>{permissionKey === 'MANAGE_STATION' ? intl.formatMessage({ id: 'stations' })
           : permissionKey === 'MANAGE_STAFF' ? intl.formatMessage({ id: 'admin' })
           : permissionKey === 'MANAGE_USER' ? intl.formatMessage({ id: 'User' })
           : permissionKey === 'MANAGE_VEHICLE' ? intl.formatMessage({ id: 'Vehicle' })
           : permissionKey === 'MANAGE_SCHEDULES' ? intl.formatMessage({ id: 'schedules' })
           : permissionKey === 'MANAGE_DOCUMENT' ? intl.formatMessage({ id: 'documentary' })
           : permissionKey === 'MANAGE_STATION_STAFF' ? intl.formatMessage({ id: 'technicians' })
           : permissionKey === 'MANAGE_STATION_USER' ? intl.formatMessage({ id: 'staff' })
           : permissionKey === 'MANAGE_CHAT' ? intl.formatMessage({ id: 'Chat' })
           : permissionKey === 'MANAGE_NOTIFICATION' ? intl.formatMessage({ id: 'notification' })
           : permissionKey === 'MANAGE_STATION_PAYMENT' ? intl.formatMessage({ id: 'pay' })
           : '-'}</>
        )
      }
    },
    {
      ...
      cell: (row) => {
        const { permissionKey } = row
        const isChecked = state.data.permissions.indexOf(permissionKey) > -1
        return (
          <>
            <Input 
            checked={isChecked}
