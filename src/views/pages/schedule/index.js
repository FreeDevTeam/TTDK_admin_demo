...
const ListSchedule = ({ intl }) => {
        ...
        return (
          <div>
            {scheduleType === SCHEDULE_TYPE.VEHICLE_INSPECTION
              ...
              ? <Badge color='light-success' className='size_text'>{intl.formatMessage({ id: 'CONSULTANT_MAINTENANCE' })}</Badge>
              : scheduleType === SCHEDULE_TYPE.CONSULTANT_INSURANCE
              ? <Badge color='light-success' className='size_text'>{intl.formatMessage({ id: 'CONSULTANT_INSURANCE' })}</Badge>
              : scheduleType === SCHEDULE_TYPE.CONSULTANT_RENOVATION
              ? <Badge color='light-success' className='size_text'>{intl.formatMessage({ id: 'CONSULTANT_RENOVATION' })}</Badge>
              : '-'}
          </div>
        )
      }
  ...
  const onExportExcel = async () => {
    ...
    async function fetchData(param) {
      ...
      const response = await StationFunctions.getList(paramsFilter)
    ...
    while (true) {
      ...
      const convertedData = results.map(el => {
        ...
        switch (scheduleType) {
          case SCHEDULE_TYPE.VEHICLE_INSPECTION:
            services = intl.formatMessage({ id: 'old_car_registration' })
            break;
          case SCHEDULE_TYPE.NEW_VEHICLE_INSPECTION:
            services = intl.formatMessage({ id: 'new_car_registration' })
            break;
          case SCHEDULE_TYPE.REGISTER_NEW_VEHICLE:
            services = intl.formatMessage({ id: 'new_car_profile' })
            break;
          default:
            break;
        }
          ...
          <Form
            ...
            <FormGroup>
              ...
              <BasicAutoCompleteDropdown  
                ...
                options={listNewStation}
                onChange={({ value }) => {
                  setUpdate({...update, stationsId : value})
                  setDates(false)
                  // getListDate(value)
                }}
