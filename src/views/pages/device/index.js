        ...
        return (
          <div>
            {deviceStatus === DEVICE_STATUS.NEW
              ? <Badge color='light-warning' className='size_text'>{intl.formatMessage({ id: 'new' })}</Badge>
              : deviceStatus === DEVICE_STATUS.ACTIVE
              ? <Badge color='light-success' className='size_text'>{intl.formatMessage({ id: 'active' })}</Badge>
              : deviceStatus === DEVICE_STATUS.MAINTENANCE
              ? <Badge color='light-primary' className='size_text'>{intl.formatMessage({ id: 'maintenance' })}</Badge>
              : deviceStatus === DEVICE_STATUS.MAINTENANCE_SERVICE
              ? <Badge color='light-info' className='size_text'>{intl.formatMessage({ id: 'maintenance_service' })}</Badge>
              : deviceStatus === DEVICE_STATUS.REPAIR
              ? <Badge color='light-secondary' className='size_text'>{intl.formatMessage({ id: 'repair' })}</Badge>
              : <Badge color='light-danger' className='size_text'>{intl.formatMessage({ id: 'inactive' })}</Badge>}
          </div>
        )
      }
    },
    ...
    <Fragment>
      <Card>
        <Row className="mx-0 mt-1 mb-50">
          <Col className="d-flex mt-sm-0 mt-1" sm='6' md = '4' lg='2' xs='12'>
          ...
          <Col sm='4' md = '3' lg='2' xs='6' className='mb-1'>
            <Button.Ripple
              color="primary"
              type="button"
              size={SIZE_INPUT}
              className="mr-1 style_add"
              onClick={(e) => {
                e.preventDefault()
...
const FIELD_IMPORT_FULL = ["Mã trạm", "Tên thiết bị", "Tình trạng", "Loại", "Số Seri", "Nhãn hiệu", "Năm sản xuất" , "Ngày kiểm chuẩn thiết bị" , "Ngày hết hạn kiểm chuẩn thiết bị"]
const FIELD_IMPORT_API = ['stationsId', 'deviceName', 'deviceStatus', 'deviceType', 'deviceSeri', 'deviceBrand', 'deviceManufactureYear' , "deviceTestedDate" , "deviceExpiredTestedDate"]
