...
import { MoreVertical, Edit, Lock, Shield } from 'react-feather'
...
const DataTableServerSide = ({ intl }) => {
          ...
          <UncontrolledDropdown>
            ...
            <DropdownMenu right>
              ...
              <DropdownItem href='/' onClick={e => {
                ...
                <Shield className='mr-50' size={15} /> <span className='align-middle'>
  ...
  function getData(params, isNoLoading) {
    ...
    if (token) {
      ...
      Service.send({
        method: 'POST', path: 'AppUsers/getListlUser', data: newParams, query: null, headers: {
  ...
  const onChangeUserPassword  = (data) => {
    Service.send({
      method: 'POST', path: 'Staff/changePasswordUser', data, query: null
    }).then(res => {
      if (res) {
        ...
        if (statusCode === 200) {
          ...
          setUserData({})
          toast.success(intl.formatMessage({ id:'actionSuccess' }, {action: intl.formatMessage({id: "resetPass"})}))
        } else {
          toast.warn(intl.formatMessage({ id:'actionFailed' }, {action: intl.formatMessage({id: "resetPass"})}))
        }
      } else {
      }
    })
  ...
  return (
    <Fragment>
                ...
                <DropdownMenu>
                  {
                    List_Search_Filter.map(text => (
                      <DropdownItem className="dropdownItem-search" value={text} onClick={() => { handleChangeSearchField(text) }} key={text}>
      ...
      <Modal
        ...
        toggle={() => setIsResetPassword(false)}
        className={`modal-dialog-centered `}
      >
        <ModalHeader toggle={() => setModal(false)}>
          {intl.formatMessage({id: "resetPass"})}
