...
const DataTableServerSide = ({ intl }) => {
  ...
  const serverSideColumns = [
    ...
    },
    {
      ...
      cell: (row) => {
        ...
        return (
          <div>
            {active === ACTIVE_STATUS ? intl.formatMessage({id:'locked'}) : intl.formatMessage({id:"ok"})}
          </div>
        )
      }
    },
  ...
  const defaultSelect = userData?.stationArea?.map(el =>{
  ...
  const ModalSwal = (newData) => {
    return MySwal.fire({
      title: intl.formatMessage({ id: 'do_you_lock' }),
      ...
      if (result.isConfirmed) {
        Swal.fire('Deleted!', 'Your file has been deleted.', 'success', handleUpdateData(newData))
      }
    })
  }
