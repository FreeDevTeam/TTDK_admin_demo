...
import { MoreVertical, Edit, Lock, Shield, RotateCcw, User} from "react-feather";
...
const DataTableTechnician = ({ intl }) => {
  ...
  const serverSideColumns = [
    ...
    },
    {
      ...
      return (
        <div>{appUserRoleId === APP_USER_ROLE.patern ? <div className="style_role">
          <Avatar className="rounded-circle" color='light-primary' icon={<User size={18} />} />
            <h6 className="">{intl.formatMessage({ id: "patern" })}</h6>
        </div>
       : appUserRoleId === APP_USER_ROLE.technician ? <div className="style_role">
       <Avatar className="rounded-circle" color='light-success' icon={<User size={18} />} />
         <h6 className="">{intl.formatMessage({ id: "technician" })}</h6>
     </div>
       : appUserRoleId === APP_USER_ROLE.technicians_senior ? <div className="style_role">
       <Avatar className="rounded-circle" color='light-warning' icon={<User size={18} />} />
         <h6 className="">{intl.formatMessage({ id: "technicians_senior" })}</h6>
     </div>
       : appUserRoleId === APP_USER_ROLE.accountant ? <div className="style_role">
       <Avatar className="rounded-circle" color='light-info' icon={<User size={18} />} />
         <h6 className="">{intl.formatMessage({ id: "accountant" })}</h6>
     </div>
       : '-'}</div>
      )
      },
        ...
        return (
          <div>
              <span
                ...
                onClick={(e) => {
                  e.preventDefault();
                  history.push("/users/form-technicians", row)
                }}
              >
  ...
  const onExportExcel = async () => {
    ...
    async function fetchData(param) {
      const response = await CenterService.getList(
        {
          filter: {
            active: 1,
            appUserRoleId: 0
