...
const DataTableTechnician = ({ intl }) => {
  ...
  function getData(params, isNoLoading) {
    ...
    if (token) {
      ...
      if(newParams.filter.stationsId === 'lean'){
  ...
  const onExportExcel = async () => {
    ...
    async function fetchData(param) {
      ...
      if(Object.keys(paramsFilter.filter).length !== 0){
        newData.filter.stationsId = paramsFilter.filter.stationsId
      }
      const response = await CenterService.getList(newData)
      ...
      async function fetchDataStaff(param) {
        const newData = {
          filter: {
            active: 1,
            // appUserRoleId: 0
      ...
      if(Object.keys(paramsFilter.filter).length !== 0){
        ...
        const response = await CenterService.getListStaff(newData)
        const data = await response.data.data;
        return data;
      } 
        for (const param of params) {
          const result = await fetchDataStaff(param);
          arrayStaff = [...arrayStaff , ...result]
        }
      const totalResult = [...results, ...arrayStaff]
      const convertedData = totalResult.map(appUser => {
