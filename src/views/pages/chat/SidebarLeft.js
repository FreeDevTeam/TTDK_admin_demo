...
import { selectChat } from "../../../redux/actions/chat/index";
...
const SidebarLeft = ({ sidebar, handleSidebar, store, intl } = {}) => {
    ...
    const listRender = useMemo(() => {
        return chats?.data?.filter((item) => {
            ...
            return `${item?.appUserConversationId}`.includes?.(searchText);
        });
                ...
                >
                    ...
                    >
                        <h4 className="chat-list-title">
                            {intl.formatMessage({ id: "message_chat" })} ({!searchText ? chats?.total : listRender?.length})
                        </h4>
                        <ul className="chat-users-list chat-list media-list">
                            {!listRender?.length && (
                                <li className="no-results show">
                                    <h6 className="mb-0">{intl.formatMessage({ id: "no_message" })}</h6>
                                </li>
                            )}
                            {listRender?.map?.((item) => {
