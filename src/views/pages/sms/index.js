    ...
    <Fragment>
      <Card>
        <Row className="mx-0 mt-1 mb-50">
          ...
          <Col sm='2' className="mt-sm-0 mt-1 d-flex align-items-center">
            <Input onChange={(e) => {
              const { name, value } = e.target
              handleFilterChange(name, value)
            }} type='select' name='stationsId' value={paramsFilter.filter ? (paramsFilter.filter.stationsId) : ''} bsSize='sm' >
              {
                stationsOptions.map(item => {
                  return <option key={item.value} value={item.value}>{intl.formatMessage({id:item.label})}</option>
                })
              }
            </Input>
          </Col>
