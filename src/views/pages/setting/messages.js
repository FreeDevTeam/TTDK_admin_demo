...
const DefaultFilter = {
  ...
  "startDate": moment(firstDay + 1000).format("DD/MM/YYYY"),
  "endDate": moment(lastDay + 8639999).format("DD/MM/YYYY")
};
  ...
  const handleFilterChange = (name, value) => {
    if (name === "date") {
      ...
      const newFilterObj = pickBy(
        {
          ...filterObj,
          ["startDate"]: value[0] ? moment(new Date(new Date(value[0]).getTime() + ms)).format("DD/MM/YYYY") : undefined,
          ["endDate"]: value[1] ? moment(new Date(new Date(value[1]).getTime() + ms)).format("DD/MM/YYYY") : undefined,
