        ...
        {image.map((img, index) => (
          <div key={index} className="upload-col">
            ...
            <div className="image-description">
              ...
              <div>({ASPECT_RATIO_X * SCALE_FACTOR}px x {ASPECT_RATIO_Y * SCALE_FACTOR}px)</div>
            </div>
          </div>
      ...
      <Modal isOpen={modal}>
        <ModalBody>
          <Cropper
            ...
            ref={cropperRef}
            aspectRatio={(ASPECT_RATIO_X * SCALE_FACTOR) / (ASPECT_RATIO_Y * SCALE_FACTOR)}
            style={{ height: '400px', width: '100%' }}
            ready={() => {
              const cropperInstance = cropperRef.current.cropper;
              if (cropperInstance) {
                cropperInstance.setCropBoxData({ width: ASPECT_RATIO_X * SCALE_FACTOR, height: ASPECT_RATIO_Y * SCALE_FACTOR });
              }
            }}
          />
        </ModalBody>
        <ModalFooter>
          <Button color="primary" onClick={handleCrop}>
            <FormattedMessage id="setting_crop" />
          </Button>
