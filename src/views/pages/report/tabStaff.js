...
const TabStaff = ({ active, intl }) => {
  ...
  const getTotalPatern = (paramPatern) => {
    tabStaffService.getListStaffPatern(paramPatern).then((result) => {
      if (result) {
  ...
  const getTotalTechnician = (paramTechnician) => {
    tabStaffService.getListStaffTechnician(paramTechnician).then((result) => {
      if (result) {
  ...
  const getTotalHigh = (paramHigh) => {
    tabStaffService.getListStaffHigh(paramHigh).then((result) => {
      if (result) {
  ...
  const getTotalAccountant = (paramAccount) => {
    tabStaffService.getListStaffAccount(paramAccount).then((result) => {
      if (result) {
      ...
      <Card className="card-transaction">
        <CardHeader>
          ...
          <CardTitle className="font-small-3">
            {intl.formatMessage({ id: 'total_staff' })} : {total}
          </CardTitle>
        </CardHeader>
