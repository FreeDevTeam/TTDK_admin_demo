...
const Wattage = ({ intl }) => {
  ...
  area?.map((item, index) => {
    array[index] = {
      name : item.value ,
      data : readLocal?.filter((el) => el.stationArea === item.value),
    }
  })
  ...
  const resultArray = newArray?.map((item) => {
    const variable = item.data?.map((result) => {
      return (result.stationBookingConfig?.filter(value => value.enableBooking !== 0))
    }
    )
    ...
    const result = newArr.map(item => {
      const total = item.map(value => {
        totalActive += value.limitSmallCar + value.limitOtherVehicle;
      })
      return totalActive
    })
