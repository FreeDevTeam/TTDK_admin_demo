...
const Wattage = ({ intl }) => {
  ...
  const resultArray = newArray?.map((item) => {
    const variable = item.data?.filter((result) =>
    result.stationBookingConfig?.filter(value => value.enableBooking === 1 )
    )
    let totalActive = 0
    const result = variable.map(item =>{
        for(let i = 0; i< item.stationBookingConfig.length; i++){
            totalActive += item.stationBookingConfig[i].limitSmallCar;
            return totalActive
        }
    })
    const totalItem = item.data.map(value =>{return value.totalInspectionLine})
    const totalLine = totalItem.reduce((total,currentValue) => {
        return total + currentValue;
      });
      const totalExpected = totalLine * 40
    const percent = (totalActive  * 100 / totalExpected).toFixed(0)
    return {
      ...item , 
      percent 
    }
        ...
        <Card className="card-browser-states">
          <CardHeader className='flex-nowrap'>
            <div>
              ...
              <CardText className="font-small-2">{intl.formatMessage({ id: 'wattage_area' })}</CardText>
            </div>
...
export default injectIntl(memo(Wattage))
