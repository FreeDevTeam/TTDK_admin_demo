      ...
      <Card style={{paddingLeft : '15px', paddingRight : '15px'}}>
          ...
          <div className="tables">
            <table border="1" width="1500px;" height="500px" align="center">
                ...
                {item?.resultReport?.map((item, i) =>{
                  return (
                    <tr key={i}>
                       ...
                       <th>{formatDisplayNumber(item.Thu100SoLuot)}</th>
                       <th>{formatDisplayNumber(item.Thu100GiaKD)}</th>
                       <th>{formatDisplayNumber(item.Thu50SoLuot)}</th>
                       <th>{formatDisplayNumber(item.Thu50GiaKD)}</th>
                       <th>{formatDisplayNumber(item.Thu0SoLuot)}</th>
                       <th>{formatDisplayNumber(item.KiemDinhLan1Dat)}</th>
                       <th>{formatDisplayNumber(item.KiemDinhLan1KoDat)}</th>
                       <th>{formatDisplayNumber(item.KiemDinhLan2Dat)}</th>
                       <th>{formatDisplayNumber(item.KiemDinhLan2KoDat)}</th>
                       <th>{formatDisplayNumber(item.TemKiemDinhKDVT)}</th>
                       <th>{formatDisplayNumber(item.TemKiemDinhKoKDVT)}</th>
                       <th>{formatDisplayNumber(item.KiemDinhXeCuLan1)}</th>
                       <th>{formatDisplayNumber(item.KiemDinhXeCuLan2)}</th>
                    </tr>
                    )
                })}
              ...
              <tr>
                ...
                <th className='theads'>{formatDisplayNumber(item.Tong100SoLuot)}</th>
                <th className='theads'>{formatDisplayNumber(item.Tong100KD)}</th>
                <th className='theads'>{formatDisplayNumber(item.Tong50SoLuot)}</th>
                <th className='theads'>{formatDisplayNumber(item.Tong50KD)}</th>
                <th className='theads'>{formatDisplayNumber(item.TongThu0)}</th>
                <th className='theads'>{formatDisplayNumber(item.TongLan1Dat)}</th>
                <th className='theads'>{formatDisplayNumber(item.TongLan1KDat)}</th>
                <th className='theads'>{formatDisplayNumber(item.Tonglan2Dat)}</th>
                <th className='theads'>{formatDisplayNumber(item.TongLan2KDat)}</th>
                <th className='theads'>{formatDisplayNumber(item.TongTemD)}</th>
                <th className='theads'>{formatDisplayNumber(item.TongTemKhongK)}</th>
                <th className='theads'>{formatDisplayNumber(item.TongKiemDinhCuLan1)}</th>
                <th className='theads'>{formatDisplayNumber(item.TongKiemDinhCuLan2)}</th>
              </tr>
            </table>
          </div>
        </Row>
        <Row className="tables-text mb-3">
          <Col sm="12" md="6" lg="4" xs="12" className='mobie_style'>
            <div>{intl.formatMessage({ id: 'Total_passes_standards' })} {formatDisplayNumber(item.totalStandards)}</div>
            <div>{intl.formatMessage({ id: 'Total_not_passes_standards' })} {formatDisplayNumber(item.totalNotStandards)}</div>
            <div>{intl.formatMessage({ id: 'Total_number_tested' })} {formatDisplayNumber(item.totalAccreditation)}</div>
          </Col>
          <Col sm="12" md="6" lg="4" xs="12" className='mobie_style'>
            <div>{intl.formatMessage({ id: 'Total_inspection_price' })} {formatDisplayNumber(item.totalInspectionPrice)} (đồng)</div>
            <div>{intl.formatMessage({ id: 'Total_fee_issuance' })} {formatDisplayNumber(item.totalGrant)} (đồng)</div>
            <div>{intl.formatMessage({ id: 'Total_proceeds' })} {formatDisplayNumber(item.totalAmount)} (đồng)</div>
          </Col>
          <Col sm="12" md="8" lg="4" xs="12" className='mobie_style'>
            <div>{intl.formatMessage({ id: 'Number_granted' })}: {formatDisplayNumber(item.grantedTemporary)}</div>
            <div>{intl.formatMessage({ id: 'Number_of_used' })}: {formatDisplayNumber(item.carsFailed)}</div>
          </Col>
        </Row>
        <Row>
          <Col>
              ...
              <tr>
                ...
                {item?.precentVehicle?.map((item, i) =>{
                  return (
                    <tr key={i}>
                      <th>{item.Cum}</th>
                      <th>{formatDisplayNumber(item.NhanDang)}</th>
                      <th>{formatDisplayNumber(item.KhungGhe)}</th>
                      <th>{formatDisplayNumber(item.DcoHeThong)}</th>
                      <th>{formatDisplayNumber(item.TruyenLuc)}</th>
                      <th>{formatDisplayNumber(item.Phanh)}</th>
                      <th>{formatDisplayNumber(item.Lai)}</th>
                      <th>{formatDisplayNumber(item.Treo)}</th>
                      <th>{formatDisplayNumber(item.Lop)}</th>
                      <th>{formatDisplayNumber(item.Den)}</th>
                      <th>{formatDisplayNumber(item.KhiXa)}</th>
                      <th>{formatDisplayNumber(item.TiengOn)}</th>
                      <th>{formatDisplayNumber(item.htKhac)}</th>
                    </tr>
                  )
                })}
            </table>
