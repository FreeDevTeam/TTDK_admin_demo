...
const categoryTypes = [
  { value: 1, label: 'Tin tức chung' },
  { value: 3, label: 'Ưu đãi' },
  { value: 4, label: 'Tuyển dụng' },
  { value: 5, label: 'Chuyên gia chia sẻ' },
  { value: 6, label: 'Ưu đãi từ đồi tác' },
]
      ...
      <Nav tabs>
          <NavItem>
            <NavLink className={activeTab == '1' ? 'active' : ''} onClick={() => setActiveTab('1')}>
              {intl.formatMessage({ id: 'general_news' })}
            </NavLink>
          </NavItem>
        ...
        {activeTab == 1 ?<Row className="mx-0 mt-1 mb-50 pt-2">
          ...
          <Col md="2" sm="4" xs="12" className="d-flex mt-sm-0 mb-1 h-100">
            <InputGroup className="input-search-group">
              <Input onChange={(e) => {
              const { name, value } = e.target
              handleFilterChange(name, value)
            }} type='select' value={paramsFilter.filter ? (paramsFilter.filter.stationNewsCategories || '') : ''} name='stationNewsCategories' bsSize='sm' >
              {
