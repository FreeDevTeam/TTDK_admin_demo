    ...
    },
    {
      ...
      cell: (row) => {
        return (
          <>
            <a
              className="text-primary pr-1 w-25"
              onClick={() =>
                handleHideNews({
              ...
              {row.isHidden === 0 ? intl.formatMessage({ id: 'hidden' }) : intl.formatMessage({ id: 'Show' })}
            </a>
            <a className="text-primary pr-1 w-25" onClick={() => handelUpdatePost(row)}>
            ...
            <a
              ...
              {intl.formatMessage({ id: 'delete_action' })}
            </a>
          </>
        )
      }
    ...
    if (token) {
      ...
      NewsService.handleUpdateNews(data, newToken).then((res) => {
        if (res) {
          const { statusCode, data, message } = res
          if (statusCode === 200) {
            toast.success(intl.formatMessage({ id: 'update-post-succes' }))
            const newParams = {
              ...paramsFilter,
              skip: (currentPage - 1) * paramsFilter.limit
            }
